...
// NOTE (sanitized build): the original version pinned @match to a specific
// SharePoint document URL (site name + doc GUID) for one workbook. That's
// been replaced here with broad domain matches; the script's own runtime
// check below narrows it further. Point this at your own workbook by
// tightening @match or the currentUrl check as needed.
    ...
    function renderDynamicOverrideGrid() {
        ...
        lines.forEach(line => {
            ...
            if (tabs.length > 0) {
                ...
                // Added data-row index parameter to the table row container
                html += `<tr data-row="${rowIndex}" style="border-bottom: 1px solid #eee; background: #fff;">`;
                html += `<td style="padding: 4px; border-right: 1px solid #eee; font-weight: bold; font-family: monospace; background: #fafafa; font-size: 10px;">${originalRouteName}</td>`;
                ...
                for (let stopNum = 1; stopNum <= 20; stopNum++) {
                    ...
                    html += `
                        <td style="padding: 2px; border-right: 1px solid #eee; text-align: center; position: relative;">
                            <!-- EXCEL STYLE DYNAMIC FOCUS CELL WRAPPER -->
        ...
        inputs.forEach((input) => {
            ...
            // --- NEW: Dynamic Crosshair Focus Highlighter System ---
            input.addEventListener('focus', () => {
                const activeRow = input.getAttribute('data-row');
                const activeCol = input.getAttribute('data-col');

                // 1. Give the active typing cell a thick green border outline box
                input.style.border = '2px solid #107c41';
                input.style.background = '#ffffff';

                // 2. Scan every cell element to dynamically draw the row and column crosshair lines
                inputs.forEach(otherInput => {
                    const r = otherInput.getAttribute('data-row');
                    const c = otherInput.getAttribute('data-col');

                    if (otherInput !== input) {
                        // If it matches either the exact row or exact column index line, highlight it green
                        if (r === activeRow || c === activeCol) {
                            otherInput.style.background = '#d1e7dd'; // Muted soft light green track lines
            ...
            // Clean up and instantly reset layout colors when the cell loses current pointer focus
