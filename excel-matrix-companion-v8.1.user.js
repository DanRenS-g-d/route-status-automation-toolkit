    ...
    function renderDynamicOverrideGrid() {
        ...
        lines.forEach(line => {
            ...
            if (tabs.length > 0) {
                ...
                // Row container assignment block
                html += `<tr data-row="${rowIndex}" style="border-bottom: 1px solid #eee; background: #fff;">`;

                // FROZEN INDEX LAYER: This is the dedicated label column (does not collect text inputs)
                ...
                // INPUT LAYER: Generates text boxes from Stop 1 to Stop 20
        ...
        inputs.forEach((input) => {
            ...
            // UPGRADED CROSSHAIR EVENT LISTENER WITH COLUMN FREEZE TRACKING
            input.addEventListener('focus', () => {
                ...
                // Locate the parent row context to paint the locked route label cell too
            ...
            // Clean layout coloring when tracking focus leaves the cell matrix
            input.addEventListener('blur', () => {
                ...
                if (parentRow) {
                    const frozenRouteCell = parentRow.querySelector('td:first-child');
                    if (frozenRouteCell) frozenRouteCell.style.background = '#fafafa'; // Revert back to original light gray
                }
