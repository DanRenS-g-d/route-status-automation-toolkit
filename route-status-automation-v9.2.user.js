...
// ROUTE_ID_PATTERN / FLATBED_ROUTE_KEYWORD / STORE_KEYWORD_REGEX below to
// match your own naming conventions before running this against a real site.
...
(function() {
    ...
    // "0421RTE01" or "0421TRK7" or "0421FLT3".
    const ROUTE_ID_PATTERN = /\b\d+(?:RTE|TRK|FLT)\d*\b/i;

    // Substring within a route ID that flags it as a flatbed-truck route
    // rather than the default truck type. Customize to your own convention.
    const FLATBED_ROUTE_KEYWORD = "FLT";
    ...
    const parsedRoutes = [];
    lines.forEach(line => {
        // Route-ID detection uses the configurable ROUTE_ID_PATTERN above
    ...
    async function processNextRoute() {
        ...
        // --- FIXED RUNTIME SKIP INTERCEPTOR ---
        ...
        if (activeTab.userSkippedRoutes.includes(fullRoute)) {
            ...
            // Jump straight to the next route with zero clicks or layout lag
            nextIteration();
            return;
        }
        // --- END OF INTERCEPTOR ---
        ...
        try {
            ...
            // --- DYNAMIC TRUCK TYPE SWITCHER FOR BOTH PLATFORMS ---
            const isFlatBed = fullRoute.toUpperCase().includes(FLATBED_ROUTE_KEYWORD);
            const targetTruckValue = isFlatBed ? "Large Flat Bed" : "Box Truck";

            try {
                // Step 1: Locate the primary dropdown combobox div element
                const truckDropdown = document.querySelector('div[role="combobox"]');

                if (truckDropdown) {
                    const currentSelection = truckDropdown.textContent.trim();

                    // PERFORMANCE LOCK: Only trigger the click sequence if the menu is on the wrong layout type
                    if (!currentSelection.includes(targetTruckValue)) {

                        // Dispatch mouse down to trick React/Material-UI into rendering the options listbox
                        ['mousedown', 'click'].forEach(eventType => {
                            truckDropdown.dispatchEvent(new MouseEvent(eventType, {
                                view: window, bubbles: true, cancelable: true, buttons: 1
                            }));
                        });

                        // Give the framework portal 800ms to safely mount the options elements into the DOM
                        await new Promise(r => setTimeout(r, 800));

                        // Step 2: Target the explicit target menu item layout option using its data-value attribute
                        const targetOption = document.querySelector(`li[role="option"][data-value="${targetTruckValue}"]`);
                        ...
                        if (targetOption) {
                            ...
                            // REQUIRED 2-SECOND DELAY: Enforce strict operational wait for portal grids to filter
                            await new Promise(r => setTimeout(r, 2000));
                        } else {
                            console.warn(`Target option [${targetTruckValue}] missing from portal layer.`);
                        }
                    }
                } else {
                    console.warn("Primary truck type select dropdown box not found on screen.");
                }
            } catch (dropdownErr) {
                console.error("Failed to execute dual-direction fleet type selection override:", dropdownErr);
            }
            // --- END OF DYNAMIC TRUCK SWITCHER BLOCK ---

            // Step 4: Search for and dismiss the modal 'Close' button
            ...
            // Evaluate missing steps against historical entries for tracking deletions (DEL)
    ...
    function renderOutputList() {
        ...
        activeTab.routesToProcess.forEach(route => {
            ...
            // CLICKABLE ROUTE HEADER LABEL BUTTON CELL
            rowHtml += `
                <td class="edp-route-toggle-btn" data-route="${route}" style="padding: 6px; border-right: 1px solid #ddd; font-weight: bold; background: ${isUserSkipped ? '#ef9a9a' : '#fff'}; color: ${isUserSkipped ? '#b71c1c' : '#000'}; position: sticky; left: 0; border-bottom: 1px solid #eee; cursor: pointer; text-align: left; user-select: none;" title="Click to skip/include this route">
                    ${isUserSkipped ? '🚫 ' : ''}${route}
                </td>`;
        ...
        // POINT-AND-CLICK ROUTE SELECTION INTERCEPTOR EVENT TRACKING MOUSE LISTENERS
        table.querySelectorAll('.edp-route-toggle-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (routeIndex > -1) {
                    // If route is already inside the skip list array, delete it to include it back
                    activeTab.userSkippedRoutes.splice(routeIndex, 1);
                } else {
                    // Push string key directly into the isolation matrix database
                    activeTab.userSkippedRoutes.push(selectedRoute);
                }

                saveStateToStorage();
                renderOutputList(); // Immediately paint updated row states red
            });
        });
    }
    ...
    function createNewTabScope() {
        ...
        tabsState.tabs[id] = {
            ...
            stopHistory: {},
            finalOutputData: {},
            userSkippedRoutes: [] // FIXED: Holds skipped route string parameters safely
        };
